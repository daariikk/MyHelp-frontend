...
export default function SpecializationDoctorsPage({ params }: { params: { specializationID: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Получаем врачей по специализации
        const doctorsResponse = await fetch(
          `http://localhost:8082/MyHelp/specializations/${params.specializationID}`
        )
        ...
        if (doctorsData.status === 'success') {
          ...
          // Предполагаем, что первый врач содержит название специализации
    ...
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        <div className={styles.heading}>
          ...
          <BackButton />
            {loading ? 'Загрузка...' : 
             specializationName ? `Врачи по специализации: ${specializationName}` : 'Наши врачи'}
          </h2>
